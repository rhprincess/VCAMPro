...
class HookMain : IYukiHookXposedInit {
    ...
    override fun onHook() = YukiHookAPI.encase {
        ...
        if (this.packageName == "io.twinkle.vcampro") {
            "io.twinkle.vcampro.util.ModuleStatus".toClass().method {
                name = "isActivated"
                emptyParam()
                    ...
                    if (param.args[0] is Application) {
                        ...
                        if (toastContent != null) { //后半段用于强制私有目录
                            ...
                            if (auth_statue < 1 || force_private.exists()) {
                                var shown_file = File(
                                    toastContent!!.getExternalFilesDir(null)!!.absolutePath + "/Camera1/"
                                )
                                ...
                                shown_file =
                                    File(toastContent!!.getExternalFilesDir(null)!!.absolutePath + "/Camera1/" + "has_shown")
                                val toast_force_file =
                                    File(Environment.getExternalStorageDirectory().path + "/DCIM/Camera1/force_show.jpg")
                                if (this@encase.packageName != BuildConfig.APPLICATION_ID && (!shown_file.exists() || toast_force_file.exists())) {
                                    try {
                                        Toast.makeText(
                                            toastContent, """
     ...
     """.trimIndent() + toastContent!!.getExternalFilesDir(null)!!
                                                .absolutePath + "/Camera1/", Toast.LENGTH_SHORT
                                        ).show()
                                        val fos = FileOutputStream(
                                            toastContent!!.getExternalFilesDir(null)!!.absolutePath + "/Camera1/" + "has_shown"
                                        )
                                        val info = "shown"
